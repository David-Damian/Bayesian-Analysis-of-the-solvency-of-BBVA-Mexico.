...
```{r}
#DATOS
datos_cartera <- read_excel("carteraP_2022.xlsx", sheet = 1, col_names = TRUE, col_types = NULL, na = "", skip = 0)
datos_captacion <- read_excel("captacionP_2022.xlsx", sheet = 1, col_names = TRUE, col_types = NULL, na = "", skip = 0)

#Variables explicativas
variables_m <- read.table("var_baseM_2022.txt", sep = "\t", header = TRUE)
variables_t <- read.table("var_baseT_2022.txt", sep = "\t", header = TRUE)
```
...
```{r}
df_research_m <- read.table('var_baseM_2022.txt',sep='\t',header = TRUE)
df_research_t <- read.table('var_baseT_2022.txt',sep='\t',header = TRUE) %>% select(Fechas,Stock_market, S.P500,Tasa_desempleo,Exportaciones_NO_Petroleras)

cartera <- read_excel("carteraP_2022.xlsx", sheet = 1, col_names = TRUE, col_types = NULL, na = "", skip = 0)
...
# variables_m1 %>% ggplot(aes(x=Fecha, y=scale(HIP))) + geom_line() +
#   geom_line(aes(x=Fecha, y =scale(Tasa_BANXICO)), col = 'red') +
#   ggtitle("Correlación Cartera PyME vs PIB")
  

variables_transm1 %>% ggplot(aes(x=Fecha, y=scale(HIP))) + geom_line() +
  geom_line(aes(x=Fecha, y =scale(Treasury_10y)), col = 'red') +
  ggtitle("Correlación Cartera Hipotecaria vs T-note 10 años datos escalados")

variables_transm1 %>% ggplot(aes(x=Fecha, y=scale(HIP))) + geom_line() +
  geom_line(aes(x=Fecha, y =scale(PIB)), col = 'red') +
  ggtitle("Correlación Cartera Hipotecaria vs PIB")
...
initsa<-function(){list(alpha=0,beta=rep(0,28),tau=1,yf1=rep(1,n),yf2 = rep(1, m))} #,yf2 = rep(1, m)
parameters<-c("alpha","beta","tau","yf1","yf2") #,"yf2"
...
```
Ajuste modelo estático reducido
...
initsb<-function(){list(alpha=0,beta=rep(0,4),tau=1,yf1=rep(1,n),yf2 = rep(1, m))} #,yf2 = rep(1, m)
parameters<-c("alpha","beta","tau","yf1","yf2") #,"yf2"
...
```{r}
proyecciones<-read.csv("proyecciones.csv")

y <- proyecciones[,2]
yf1_0 <- proyecciones[,3][1:169]
yf1 <- proyecciones[,3][170:181]
yf3 <- proyecciones[,4][170:181]
yf7 <- proyecciones[,5][170:181]
